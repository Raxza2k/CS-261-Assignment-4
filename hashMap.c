...
int* hashMapGet(struct HashMap* map, const char* key)
{
    ...
    int idx = abs(HASH_FUNCTION(key)) % map->capacity;
    if(idx < 0){
        idx += map->capacity;
    }

            ptr = map->table[idx];                                              //starts iterating at the first link after the bucket sentinel.
    ...
    // FIXME: implement
    return 0;                //if key doesn't match any in table, returns NULL
}
...
if(hashMapContainsKey(map, key) == 1){
    struct HashLink* curr = map->table[idx];
    while(curr != 0){
...
void hashMapRemove(struct HashMap* map, const char* key)
{
    ...
    prev = cur;                                                     //Track your previous pointer.
